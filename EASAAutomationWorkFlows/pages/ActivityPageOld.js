...
export class ActivityPageOld {
  ...
  async createActivity(data) {
    ...
    await this.page.getByRole('cell', { name: String(data.EndDay) }).nth(1).click();
    ...
    //---Click on combo box to select the location---
    await this.page.locator('.combo-box > button').first().click();

    //---Enter the location as provided in the excel---
    await this.page.getByText(data.Location).click();

    //---Click on Submit button to add the location---
    await this.page.getByRole('button', { name: 'Submit' }).click();
